...
{    
        ...
        for (int i = 0; i < spawnList.Count; i++)
        {
            ...
            for (int j = 0; j < (int) numToSpawn; ++j)
            {
                ...
                if (OnGroundAndNotOverlapping(pos))
                {
                    ...
                    o.transform.localScale = new Vector3(Random.Range(.5f, 1.2f), Random.Range(.5f, 1.2f), Random.Range(.5f, 1.2f));
                }
            }
        }
    }
    ...
    private Vector3 GetRandomPos()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(randpos + new Vector3(0, 50f, 0), Vector3.down, out hit, 100f))
